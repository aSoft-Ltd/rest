...
internal suspend fun <T : Entity> IRestModule<T>.loadOne(call: ApplicationCall, log: Logger) = try {
    val id = call.parameters["uid"].toString()
    when (val state = authorize(call, log,"load entity(uid=$id)", ":read")) {
        is AuthorizationState.UnAuthorized -> {
            send(call, log, state.code, serializer, state.res)
        }
        is AuthorizationState.Authorized -> {
            val entity = controller.load(id)
